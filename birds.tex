...
\begin{titlepage}
    ...
    \textit{Honors Calc I Fall 2024 Project Final}
...
The goal is to understand how speed influences energy use and to find the speeds where birds fly most efficiently. These ideas are similar to the way fixed-wing planes balance power and fuel efficiency. By studying birds, we can learn more about energy use in both nature and human-made systems.

In this project, we use ideas from calculus to analyze these patterns. Specifically, we use the first derivative to find points where power use is at its highest or lowest. Then, we use the second derivative to figure out whether these points represent efficient speeds or not. These concepts of optimization, covered in Chapter 4.7, help us solve problems by using math to find the optimized points in a system or vice versa.

This project demonstrates how calculus can solve real-world problems. By applying what we’ve learned from calculus to birds in flight, we can better understand how math explains energy use in nature. It also shows how the same tools we use in class can apply to other areas, like science and engineering.
...
Thus, when \( P'(v) = 0 \) and \( P''(v) > 0 \), \( P \) is concave up, indicating an absolute minimum. Therefore, when
\[
...
To address this problem, first, let's break it down:
 ...
 But birds keep flying, more accurately, they keep gliding. During the folding stage, the bird doesn't spend more energy; instead, it uses the remaining energy from the flapping stage.

Therefore, the total energy during the whole flying process is equal to the energy during flapping, and by the physics definition, the relationship between power \(P\) and energy \(E\) is:
...
And then,let's come back to the definition of power:
 ...
 Therefore:
\[\overline{P}=xP_\text{flap}+P_\text{fold}(1-x)=((A_b+A_w)v^3+\frac{B(\frac{mg}{x})^2}{v})x+(A_bv^3)(1-x)\] 
...
Surprisingly, we find that when \(x = \frac{mg}{v^2}\sqrt{\frac{B}{A_w}}\), it minimizes \(\overline{E}\), which is the same as the value of \(x\) that minimizes the average power.
...
\textbf{\textit{But}}, the real world is far more complicated than our model. The bird's flight is influenced by many factors, such as the wind, the temperature, the humidity, and the bird's physical condition. If a bird flies too slowly, it needs more energy to maintain lift and stay aloft; if the bird flies too fast, the air resistance increases significantly. Neither extreme is the optimal survival strategy for the bird. I really wanted to discuss the optimal flying strategy, but the wisdom hidden inside the body of a tiny bird is far beyond my understanding. Therefore, the bird's flight strategy is not determined by speed alone. These findings make me once again appreciate the complexity of nature and the beauty of mathematics.
